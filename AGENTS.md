...
- `data/requirement-overrides.json` — small, sourced runtime corrections for cases where Fireroad's requirement feed lags an official departmental degree-audit chart. Keep each override narrow and include its authoritative source and effective catalog date.
...
- `data/special_subjects.json` — generated EECS special-subject overlay. `scripts/fetch_special_subjects.py --term ...` discovers the official EECS Subject Updates page and public EECSIS feed, then overlays term-specific topic names, descriptions, metadata, and source links on canonical Fireroad records while enforcing the matching offering season.
- `data/special_subject_names.json` — hand-maintained fallback map of special-subject course ids to topic names. Live official EECS term data wins when available because special-subject numbers can be reused for different topics.
- `data/substitutions.json` — curated advisory petition/substitution examples. They never auto-credit requirements because MIT EECS decides each petition case by case.
...
- The agent's `evaluate_substitution` tool is advisory, never predicts approval, and never emits a plan mutation; students must consult their advisor and submit through the department audit.
