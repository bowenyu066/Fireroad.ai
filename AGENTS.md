...
- `data/special_subjects.json` — generated EECS special-subject overlay. It can override generic entries in `data/courses.json` with curated per-term topic names; `scripts/fetch_special_subjects.py --term ...` infers and enforces the matching offering season.
- `data/special_subject_names.json` — hand-maintained map of special-subject course ids to per-term topic names.
