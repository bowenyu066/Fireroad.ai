...
- `data/special_subjects.json` — EECS special-subjects overlay (course numbers with an `S` after the dot, e.g. `6.S062`). The generator discovers the official EECS Subject Updates page and public EECSIS feed, then overlays term-specific topic names, descriptions, metadata, and source links on full Fireroad records. Generated by `scripts/fetch_special_subjects.py`; merged into the live catalog by `server/current/fireroad.js` (overlay records win by id). Override path with `SPECIAL_SUBJECTS_PATH`.
- `data/special_subject_names.json` — small hand-maintained fallback map of `{ "6.S062": "Topic title" }`. Live official EECS term data wins when available because a special-subject number can be reused for a different topic. A saved Subject Updates page can still be parsed with `--page` for offline recovery.
...
The EECS Subject Updates WordPress page is JS-rendered, but it exposes the public EECSIS HTML feed URL used by the page. The generator discovers and parses that feed directly. It includes only headings whose current number begins with `6.S`/`18.S`, so permanent courses that merely mention an old special-subject number are not reintroduced under that old number. Subjects without official or curated names keep the API's generic title and are flagged `has_real_title: false`; the catalog exposes this as `hasRealTitle` so the UI/agent can say the topic is not yet listed.
