// fireroad.ai — mock data
window.FRDATA = (function () {
  ...
  // Course catalog (small subset for demo)
  const catalog = [
    {
      id: '6.3900',
  ...
  // Minimal stubs for already-taken courses (so 4-year grid renders them)
  const takenStubs = [
  ...
  takenStubs.forEach((s) => {
    if (!catalog.find((c) => c.id === s.id)) {
      catalog.push({
        ...s, schedule: '—', days: [], time: { start: 0, end: 0 },
        instructor: '—', satisfies: [], prereqs: [], hydrant: 0,
        rating: { overall: 0, lectures: 0, difficulty: 0, n: 0 },
        desc: '', topics: [], quote: '', _stub: true,
      });
    }
  });
  ...
  // Match scores per course (would come from /api/score-courses)
  const matchScores = {
  ...
  // 4-year plan starter (taken courses already placed)
  const fourYearPlan = {
  ...
  const semesterLabels = {
    'F23': 'Fall 2023', 'S24': 'Spring 2024',
    'F24': 'Fall 2024', 'S25': 'Spring 2025',
    'F25': 'Fall 2025', 'S26': 'Spring 2026',
    'F26': 'Fall 2026', 'S27': 'Spring 2027',
  };
  const semesterOrder = ['F23','S24','F24','S25','F25','S26','F26','S27'];

  // Initial agent message bank for the planner
  const agentMessages = [
    {
      role: 'agent',
  ...
  return {
    ...
    agentMessages,
    allReqs,
    getCourse: (id) => catalog.find((c) => c.id === id),
    getMatch: (id) => matchScores[id] || { total: 0, interest: 0, workload: 0, reqValue: 0 },
  };
})();
